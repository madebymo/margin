...
python -m tutor.packs.ingest --sources ~/corpus --dry-run                     # RAG retrieval preview per KC (no LLM)
python -m tutor.packs.ingest --sources ~/corpus --kcs kc.der.chain_rule      # compile draft packs (needs OPENAI_API_KEY)
...
Four `runtime_checkable` Protocols: `DiagnosticianPort`/`LessonWriterPort` (`ports.py`) and `InteractionGeneratorPort`/`EvaluatorPort` (`planner.py`). The `Template*` implementations are deterministic, LLM-free, built from KC canonical examples — **these are the defaults**, so the whole control plane (including interactive widgets) runs and is tested offline. The `tutor/llm/` adapters (`LLMDiagnostician`, `LLMLessonWriter`, `LLMInteractionGenerator`, `LLMEvaluator`) satisfy the same Protocols and are constructed together by `build_llm_ports` (`factory.py`, returns the `LLMPorts` dataclass), shared by the CLI and API.
...
- Widget candidates are schema-validated through the widget discriminated union, pass the deterministic gates, then the evaluator acceptance rule (all hard gates true, no abstention, soft-axis min ≥ 3 and mean ≥ 4); rejection feedback drives the repair loop and exhaustion falls back to a static worked example.
...
- `packs/` — pedagogy pack CSV import plus the RAG ingestion pipeline. The CSV is a **validated import surface only** (the DB is authoritative); multi-value cells use `|`. `packs/ingest.py` is the offline pack builder: dependency-free BM25 retrieval over a local `.md`/`.txt` corpus + LLM compilation into schema-valid drafts (`review_status=llm_generated`) written to `packs/generated/`. Misconception/metaphor ids are built server-side from model slugs (`m.<kc>.<slug>` / `met.<slug>`) so membership validation stays intact; failed drafts are skipped, never fabricated. `packs/loader.py` `load_packs()` merges generated drafts with the human template CSV — **human-authored packs win on conflict** — and is what `build_llm_ports` loads.
...
- `db/persistence.py` `PersistenceService` — synchronous facade: `ensure_learner` (idempotent), `start_episode`/`update_episode` (phase + envelope checkpoints), append-only `record_event` + `load_events` (replay; naive SQLite datetimes normalized to UTC), `save_derived` upsert. The orchestrator persists when constructed with a service (API: `DATABASE_URL` or `create_app(database_url=...)`; CLI: `--db`); persistence failures log and disable for that session — they never block it.
