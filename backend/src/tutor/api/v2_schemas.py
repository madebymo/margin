...
class CoachingCapabilityView(BaseModel):
    """Whether this deployment can provide attributed OpenAI coaching."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    available: bool
    provider: Literal["openai"] | None = None
    model: Literal["gpt-5.6"] | None = None
    reason: str

    @model_validator(mode="after")
    def _availability_matches_provider(self) -> "CoachingCapabilityView":
        if self.available and (self.provider is None or self.model is None):
            raise ValueError("available coaching requires provider and model")
        if not self.available and (self.provider is not None or self.model is not None):
            raise ValueError("unavailable coaching cannot advertise a provider")
        return self
...
class CoachingAttributionView(BaseModel):
    """Public provenance for prose generated by the optional OpenAI coach."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    provider: Literal["openai"]
    model: Literal["gpt-5.6-terra", "gpt-5.6-sol"]
    policy_version: Literal["coach-v1"]
...
class TranscriptEntry(BaseModel):
    ...
    generated_by: CoachingAttributionView | None = None
...
class CreateSessionV2Request(BaseModel):
    ...
    # API v2 has one reviewed coaching implementation. Keep the legacy v1
    # provider choice separate rather than silently routing a different
    # provider request through OpenAI.
