"""Build the private reviewer packet for the pending FTC content wave.

The command compiles the exact 78 assessment families and six pedagogy sources
into deterministic truth-bearing JSON/HTML.  It does not approve families,
attest KCs, or produce a release bundle.
"""
...
def build_pending_ftc_packet(
    graph: GraphDocument,
    assessment_source: FTCBlueprintDocument,
    assessment_reviews: ContentReviewManifest,
    pedagogy_source: PedagogySourceDocument,
    pedagogy_reviews: PedagogyReviewManifest,
) -> dict[str, object]:
    """Compile exact pending FTC inputs for independent human review."""
    ...
    return build_pending_reviewer_packet(
        ...
        pedagogy_source,
        pedagogy_reviews,
        artifact_kind="pending_ftc_review",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        packet = build_pending_ftc_packet(
            graph,
        ...
        print(f"FTC review packet INVALID: {exc}", file=sys.stderr)
        return 1

    if args.check:
        print(
            "FTC review packet OK: "
