"""Build the private reviewer packet for the pending Chain Rule wave.

The command compiles the exact 39-family assessment source and three pedagogy
...
from tutor.schemas.chain_rule_authoring import ChainRuleBlueprintDocument
...
DEFAULT_PEDAGOGY_SOURCE_PATH = (
    SEED_DIR / "pedagogy_pack_sources_chain_rule_v2.json"
)
DEFAULT_PEDAGOGY_REVIEW_PATH = (
    SEED_DIR / "pedagogy_pack_reviews_chain_rule_v2.json"
)


def build_pending_chain_rule_packet(
    graph: GraphDocument,
    assessment_source: ChainRuleBlueprintDocument,
    assessment_reviews: ContentReviewManifest,
    pedagogy_source: PedagogySourceDocument,
    pedagogy_reviews: PedagogyReviewManifest,
) -> dict[str, object]:
    """Compile exact pending Chain Rule inputs for human review."""
    ...
    return build_pending_reviewer_packet(
        ...
        pedagogy_source,
        pedagogy_reviews,
        artifact_kind="pending_chain_rule_review",
...
def main(argv: list[str] | None = None) -> int:
    """Validate or write the exact pending Chain Rule review packet."""
    ...
    try:
        ...
        packet = build_pending_chain_rule_packet(
            graph,
        ...
        print(f"Chain Rule review packet INVALID: {exc}", file=sys.stderr)
        return 1

    if args.check:
        print(
            "Chain Rule review packet OK: "
