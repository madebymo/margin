"""Private reviewer-packet tests for the pending FTC wave."""
...
from tutor.content.ftc_release import load_manifest, load_source
from tutor.content.ftc_reviewer_packet import (
    DEFAULT_ASSESSMENT_REVIEW_PATH,
    DEFAULT_ASSESSMENT_SOURCE_PATH,
    DEFAULT_PEDAGOGY_REVIEW_PATH,
    DEFAULT_PEDAGOGY_SOURCE_PATH,
    build_pending_ftc_packet,
    main,
)
...
def packet(pending_inputs):
    return build_pending_ftc_packet(*pending_inputs)


def test_packet_is_private_complete_and_truth_bearing(packet):
    assert packet["schema_version"] == 2
    assert packet["artifact_kind"] == "pending_ftc_review"
    ...
    assert packet["review_workflow"]["assessment_decisions"] == {"pending": 78}
    assert packet["review_workflow"]["pedagogy_decisions"] == {"pending": 6}
    assert len(packet["families"]) == 78
    assert len(packet["pedagogy_packs"]) == 6
    assert len(packet["knowledge_components"]) == 6
    assert len(packet["first_two_paths"]) == 30
    assert packet["separation_report"] == {
        "answer_pairs_checked": 3003,
        "visible_candidate_comparisons_checked": 18547,
        "literal_visible_pairs_checked": 6006,
        "errors": [],
    }
    ...
    for family in packet["families"]:
        ...
        assert "evidence_family_independence" in family["review_requirements"]
        item = family["items"][0]
...
def test_packet_bytes_are_deterministic(packet, pending_inputs):
    rebuilt = build_pending_ftc_packet(*pending_inputs)
    ...
    packet_digest = unsigned.pop("packet_digest")
    assert canonical_digest(unsigned) == packet_digest


def test_packet_cli_writes_without_mutating_review_inputs(tmp_path, capsys):
    source_paths = (
        DEFAULT_ASSESSMENT_SOURCE_PATH,
        DEFAULT_ASSESSMENT_REVIEW_PATH,
        DEFAULT_PEDAGOGY_SOURCE_PATH,
    ...
    destination = tmp_path / "ftc-review"
    ...
    assert "78 draft families" in capsys.readouterr().out
