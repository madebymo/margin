"""Private deterministic review packets for the pending Chain Rule wave."""
...
from tutor.content.chain_rule_release import load_manifest, load_source
from tutor.content.chain_rule_reviewer_packet import (
    DEFAULT_ASSESSMENT_REVIEW_PATH,
    DEFAULT_ASSESSMENT_SOURCE_PATH,
    DEFAULT_PEDAGOGY_REVIEW_PATH,
    DEFAULT_PEDAGOGY_SOURCE_PATH,
    build_pending_chain_rule_packet,
    main,
)
...
def packet(pending_inputs):
    return build_pending_chain_rule_packet(*pending_inputs)


def test_chain_packet_is_private_complete_and_truth_bearing(packet):
    assert packet["schema_version"] == 2
    assert packet["artifact_kind"] == "pending_chain_rule_review"
    ...
    assert packet["review_workflow"]["assessment_decisions"] == {"pending": 39}
    assert packet["review_workflow"]["pedagogy_decisions"] == {"pending": 3}
    assert len(packet["families"]) == 39
    assert len(packet["pedagogy_packs"]) == 3
    assert len(packet["knowledge_components"]) == 3
    assert len(packet["first_two_paths"]) == 15
    assert all(
        "evidence_family_independence" in family["review_requirements"]
        for family in packet["families"]
    )
    assert packet["separation_report"] == {
        "answer_pairs_checked": 741,
        "visible_candidate_comparisons_checked": 8313,
        "literal_visible_pairs_checked": 1482,
        "errors": [],
    }
...
def test_chain_packet_bytes_are_deterministic(packet, pending_inputs):
    rebuilt = build_pending_chain_rule_packet(*pending_inputs)
...
def test_chain_packet_cli_writes_without_mutating_review_inputs(tmp_path, capsys):
    source_paths = (
        DEFAULT_ASSESSMENT_SOURCE_PATH,
        DEFAULT_ASSESSMENT_REVIEW_PATH,
        DEFAULT_PEDAGOGY_SOURCE_PATH,
    ...
    destination = tmp_path / "chain-rule-review"
    ...
    assert "39 draft families" in capsys.readouterr().out
